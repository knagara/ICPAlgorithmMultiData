#ifndef ICP_NORMAL_INDEX_H_INCLUDED
#define ICP_NORMAL_INDEX_H_INCLUDED
...
public:
	...
	int * index;	///ï¿Ç—ë÷Ç¶å„ÇÃóvëfî‘çÜÇäiî[Ç∑ÇÈîzóÒ
	float * distance;	///ãóó£Ç™äiî[Ç≥ÇÍÇÈÉxÉNÉgÉã
	...
	bool reg(int max_iter=100, double tau=1.0e-6)
	{
		...
		///indexÉÅÉÇÉäämï€
		index = (int *) malloc(sizeof(int) * data_shape_rt.rows);
		///distanceÉÅÉÇÉäämï€
		distance = (float *) malloc(sizeof(float) * data_shape_rt.rows);
		for(int iiter=0; iiter<max_iter; iiter++, iter++)
		{
			_model_shape.query(data_shape_rt, model_corr, index, distance);
...
public:
	...
	void query(const Mat& query_points, Mat& corr, int * myIndex, float * myDist)
	{
		...
		for(int i_row=0; i_row<num; i_row++)
		{
			...
			///indexÇ…óvëfî‘çÜÇäiî[
			myIndex[i_row] = indices.at<int>(i_row, 0);
			///distanceÇ…ãóó£Çäiî[
			myDist[i_row] = dists.at<float>(i_row, 0);
		}
	}
};

#endif ICP_NORMAL_INDEX_H_INCLUDED